...
int Thread::start(int stackSize) {
    if (m_bRunning) {
        LOG_WARNING("Try to start an already started thread.");
        return -1;
    }
    ...
    if (stackSize > 0) {
        ...
        if (0 != rt) {
            LOG_ERROR("Syscall Error: pthread_attr_init.");
            return -1;
        }
        ...
        if (0 != rt) {
            LOG_ERROR("Syscall Error: pthread_attr_setstatcksize.");
            return -1;
        }
        ...
        if (0 != rt) {
            LOG_ERROR("Syscall Error: pthread_create.");
            return -1;
        }
    } else {
        ...
        if (0 != rt) {
            LOG_ERROR("Syscall Error: pthread_create. %s", strerror(errno));
            return -1;
        }
    }
...
int Thread::join(void **valuePtr) {
    if (m_bDetached) {
        LOG_WARNING("Try to join a detached thread.");
        return -1;
    }
    ...
    if (0 != rt) {
        LOG_ERROR("Syscall Error: pthread_join.");
        return -1;
    }
...
int Thread::detach() {
    if (m_bDetached) {
        LOG_WARNING("Try to detach an already detached thread.");
        return -1;
    }
    ...
    if (0 != rt) {
        LOG_ERROR("Syscall Error: pthread_detach.");
        return -1;
    }
