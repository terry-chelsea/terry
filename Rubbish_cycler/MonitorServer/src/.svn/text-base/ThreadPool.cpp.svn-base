...
void ThreadPool::ThreadHandler::run()
{
    ...
    while(1)
    {
        ...
        LOG_EVENT("read request counter : " + int_to_str(request_counter));
...
int ThreadPool::start(int num)
{
    ...
    if(ret < 0)
    {
        LOG_SYSCALL_ERROR("ThreadPool::Create pipe for thread error !");
        return -1;
    }
    ...
    if(ret < 0)
    {
        LOG_SYSCALL_ERROR("ThreadPool::Create pipe for main error !");
