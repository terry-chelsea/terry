...
const uint32_t RULE_NOT_INITED = 0;
const uint32_t RULE_HAS_INITED = 1;
...
const struct OrderRule EMPTY_VERSION = {UNDEFINED , 0};
...
class RulerManager : public Singleton<RulerManager>
{
    ...
    public : 
        ...
        void init_system_rule(RULE_TYPE type)
        {
            if(MU_RULER == type)
            {
                m_current_version[type] = 0;
                m_current_mod[type] = 0;
                m_current_bucket_number[type] = 0;
                m_mu_hash_table.clear();
                m_mu_order_table.clear();
            }
            if(SU_RULER == type)
            {
                m_current_version[type] = 0;
                m_current_mod[type] = 0;
                m_current_bucket_number[type] = 0;
                m_su_hash_table.clear();
                m_su_order_table.clear();
            }
        }
        ...
        bool should_init(RULE_TYPE type)
        {
            return m_current_bucket_number[type] == 0;
        }

        int get_dup_num(RULE_TYPE type)
        {
            struct HashRule rule;
            if(MU_RULER == type) 
            {
                rule = m_mu_hash_table[0];
            }    
            else
            {
                rule = m_su_hash_table[0];
            }
            int dup_num = 0;
            for(int i = 0 ; i < MAX_IP_NUM ; ++ i)
            {
                if(rule.ip[i] != INVALID_IP)
                    ++ dup_num;
            }
            return dup_num;
        }
        ...
        //读取规则表中的一项
        int get_hash_rule(uint32_t bucket_nr , RULE_TYPE type , struct HashRule &);
